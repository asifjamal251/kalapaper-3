{!! html()->form('PUT', route('admin.'.request()->segment(2).'.update', $admin->id))
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    {{ html()->text('name', old('name', $admin->name))->class('form-control') }}
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                    ...
                    {{ html()->email('email', old('email', $admin->email))->class('form-control') }}
            ...
            <div class="col-md-6 col-sm-12">
                <div class="form-group{{ $errors->has('username') ? ' has-error' : '' }}">
                    ...
                    {{ html()->text('username', $admin->username)->class('form-control')->placeholder('Username')->attribute('readonly') }}
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('role') ? 'has-error' : '' }}">
                    ...
                    {{ html()->select(
                        'role',
                        App\Models\Role::whereNotIn('id',[1])->get()->pluck('role_name','id'),
                        old('role', $admin->role_id)
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
                    ...
                    {{ html()->password('password')->class('form-control')->attribute('autocomplete','new-password')->placeholder('Password') }}
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                    ...
                    {{ html()->select(
                        'status',
                        App\Models\Status::whereIn('id',[14,15])->pluck('name','id'),
                        old('status', $admin->status_id)
                    )->class('form-control js-choice') }}
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('enabled_2fa') ? 'has-error' : '' }}">
                    {{ html()->label('2FA Enabled', 'enabled_2fa') }}
                    {{ html()->select(
                        'enabled_2fa',
                        App\Models\Status::whereIn('id',[14,15])->pluck('name','id'),
                        old('enabled_2fa', $admin->google2fa_enabled)
                    )->placeholder('2FA Enabled')->class('form-control js-choice') }}
            ...
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('ip_enabled') ? 'has-error' : '' }}">
                    {{ html()->label('IP Enabled', 'ip_enabled') }}
                    {{ html()->select(
                        'ip_enabled',
                        App\Models\Status::whereIn('id',[14,15])->pluck('name','id'),
                        old('ip_enabled', $admin->ip_enabled)
                    )->class('form-control js-choice') }}
...
@if($admin->ips->count() > 0)
        <div id="kt_docs_repeater_advanced">
            <div data-repeater-list="kt_docs_repeater_advanced">

                @foreach(old('kt_docs_repeater_advanced', $admin->ips->toArray()) as $index => $item)
                <div data-repeater-item class="row">
                    ...
                    <div class="gap-4 d-flex justify-content-between flex-sm-wrape">
                            ...
                            <div class="w-100 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.ips") ? ' has-error' : '' }}">
                                ...
                                {{ html()->text("kt_docs_repeater_advanced[$index][ips]", $item['ip_address'] ?? $item['ips'] ?? '')->class('form-control numOnly')->placeholder('e.g. 192.168.1.1') }}
                            ...
                            <div class="w-100 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.notes") ? ' has-error' : '' }}">
                                ...
                                {{ html()->text("kt_docs_repeater_advanced[$index][notes]", $item['notes'] ?? '')->class('form-control')->placeholder('Office Airtel') }}
