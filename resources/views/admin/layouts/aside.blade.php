<div class="app-menu navbar-menu">
    ...
    <div class="navbar-brand-box">
        ...
        <a href="{{ route('admin.dashboard.index') }}" class="logo logo-dark">
            <span class="logo-sm">
                <img src="{{asset(get_app_setting('favicon')??'/images/favicon.png')}}" alt="" height="22">
            </span>
            <span class="logo-lg">
                <img src="{{asset(get_app_setting('logo')??'/images/logo.png')}}" alt="" height="17">
            </span>
        </a>
        <!-- Light Logo-->
        <a href="{{ route('admin.dashboard.index') }}" class="logo logo-light">
            <span class="logo-sm">
                <img src="{{asset(get_app_setting('favicon')??'/images/favicon.png')}}" alt="" width="40">
            </span>
            <span class="logo-lg">
                <img src="{{asset(get_app_setting('logo')??'/images/logo.png')}}" alt="" width="140">
            </span>
        </a>
                                            ...
                                            <div class="collapse menu-dropdown {{ $child->grands->whereIn('slug',str_replace('-', '_', request()->segment(2)))->count() ? 'show' : '' }}" id="submenu-{{ $child->slug }}">
                                                <ul class="nav nav-sm flex-column" aria-expanded="false">
                                                    @foreach($child->grands as $grand)
                                                        ...
                                                        <script>
                                                            const menuId = 'menu-{{ $menu->slug }}';
                                                            const menu = document.getElementById(menuId);

                                                            if (menu) {
                                                                menu.parentElement.classList.add('g-parent-active');
